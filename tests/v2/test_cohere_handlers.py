...
class TestCohereHandlerRegistration:
    """Tests for Cohere handler registration in the v2 registry."""
    ...
    def test_mode_is_registered(self, mode: Mode):
        """Test all Cohere modes are registered."""
        assert mode_registry.is_registered(Provider.COHERE, mode)
    ...
    def test_handlers_have_all_methods(self, mode: Mode):
        ...
        handlers = mode_registry.get_handlers(Provider.COHERE, mode)
    ...
    def test_get_modes_for_provider(self):
        """Test getting all modes for Cohere provider."""
        modes = mode_registry.get_modes_for_provider(Provider.COHERE)
    ...
    def test_parallel_tools_not_supported(self):
        """Test PARALLEL_TOOLS mode is NOT supported by Cohere."""
        assert not mode_registry.is_registered(Provider.COHERE, Mode.PARALLEL_TOOLS)

    def test_responses_tools_not_supported(self):
        """Test RESPONSES_TOOLS mode is NOT supported by Cohere."""
        assert not mode_registry.is_registered(Provider.COHERE, Mode.RESPONSES_TOOLS)
